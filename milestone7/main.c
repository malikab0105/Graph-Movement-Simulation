...
int choose_next_traveler(int node_id) {
    ...
    if (activeMode == SCHED_PRIORITY) {
        int bestIdx = 0;
        pid_t minPid = 9999999;
        for (int i = 0; i < q->waiterCount; i++) {
            int tId = q->waitingTravelers[i];
            pid_t pid = visual_travelers[tId].childPid;
            if (pid > 0 && pid < minPid) {
                minPid = pid;
                bestIdx = i;
            }
...
static void print_metrics(void) {
    ...
    const char *modeStr = (activeMode == SCHED_FCFS) ? "FCFS    " :
...
int main(int argc, char *argv[]) {
    ...
    const char *printMode = (activeMode == SCHED_FCFS) ? "FCFS" :
                            (activeMode == SCHED_SJF)  ? "SJF"  : "PRIORITY";
    printf("Scheduler: %s\n", printMode);
    printf("--------------------------------------\n\n");
    ...
    while (!WindowShouldClose()) {
        ...
        /* Scheduler banner */
        const char *modeLabel = (activeMode == SCHED_FCFS) ? "ACTIVE SCHEDULER: FCFS" :
                                (activeMode == SCHED_SJF)  ? "ACTIVE SCHEDULER: SJF" :
                                                             "ACTIVE SCHEDULER: PRIORITY";
